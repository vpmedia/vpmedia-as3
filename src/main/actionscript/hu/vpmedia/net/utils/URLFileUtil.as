...
public class URLFileUtil {
    ...
    public static function getPostData(fileName:String, byteArray:ByteArray, uploadDataFieldName:String = "Filedata", parameters:Object = null):ByteArray {
        ...
        for (var name:String in parameters) {
            if (parameters[name] != null) {
                ...
                trace("WARNING, invalid key:", name, parameters[name])
            }
        }
        //add Filedata to postData
        postData = BOUNDARY(postData);
        postData = LINEBREAK(postData);
        // WAS: bytes='Content-Disposition: form-data; name="' + uploadDataFieldName + '"; filename="' + fileName + '"';
    ...
    private static function BOUNDARY(p:ByteArray):ByteArray {
        var l:int = URLFileUtil.getBoundary().length;
        p = DOUBLEDASH(p);
